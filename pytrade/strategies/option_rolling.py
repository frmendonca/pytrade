...
def calculate_roll_details(
    ...
    post_roll_net_credit_received = (candidate_premium + current_premium_received - current_premium_to_close)*multiple
    rolled_put_value_to_breakeven = candidate_premium + (current_premium_received - current_premium_to_close)

    # Margin computations
    pre_otm_amount = max(underlying_price - current_strike, 0)
    pre_roll_margin = max(0.20*underlying_price - pre_otm_amount, 0.10*current_strike) + current_premium_received
    pre_roll_margin *= multiple

    post_otm_amount = max(underlying_price - candidate_strike, 0)
    post_roll_margin = max(0.20*underlying_price - post_otm_amount, 0.10*candidate_strike) + candidate_premium
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Calculate P&L for rolling put options.")
    parser.add_argument("--current_strike", type=float, help="Current put strike price")
    parser.add_argument("--current_premium_received", type=float, help="Premium received for current puts")
    parser.add_argument("--number_contracts_sold", type=int, help="Number of contracts")
    parser.add_argument("--current_premium_to_close", type=float, help="Current premium to close puts")
