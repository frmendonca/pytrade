...
class MixtureDistribution():
    ...
    def __init__(
        self,
        mixture_param: float,
        generators: list[RVContinuousFrozen]
    ):
    ...
    def rvs(self, size = 1):
        ...
        x = boolean_mask*self._generators[0].rvs(size) - (1 - boolean_mask)*self._generators[1].rvs(size)
        return x
...
class DiscreteConditionalDistribution():
    ...
    def rvs(self, conditional):
        ...
        for i, (lower, upper) in enumerate(zip(self._conditional_bins[:-1], self._conditional_bins[1:])):
            boolean_mask[i,:] = (conditional >= lower) & (conditional < upper)
            
        boolean_mask = np.argmax(boolean_mask, axis = 0) # Converts to 1d vector with correct entry
